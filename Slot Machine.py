...
def get_slot_machine_spin(rows, cols, symbols):
    ...
    for _ in range(cols):
        column = []
        current_symbols = all_symbols[:] # Slice operator to create a copy of the list
        for _ in range(rows):
    ...
    while True:
        ...
        if lines.isdigit():
            ...
            if 1 <= lines <= MAX_LINES:
                ...
                print("Number of lines must fit within range.")
        else:
            print("Please enter a number! ")
...
def get_bet():
    while True:
        amount = input("How much would you like to multiply on each line? $")
        if amount.isdigit():
            amount = int(amount)
